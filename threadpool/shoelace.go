...
func main() {
	...
	inputChannel := make(chan string, 1000) // buffered channel

	// pre-start the worker threads
	for i := 0; i < numberOfThreads; i++ {
		go findArea(inputChannel) // only passing the buffered channel
	}

	waitGroup.Add(numberOfThreads)
	start := time.Now()
	for _, line := range strings.Split(text, "\n") {
		inputChannel <- line // feeding each line to the channel
	}
	close(inputChannel)
	waitGroup.Wait()
